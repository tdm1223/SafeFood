...
public class FoodNutriment {
	private static final String[] serving = { "총 내용량", "1회 제공량", "1회제공량", "총 제공량" };
	...
	private static final String[] nutr = { "", "열량", "탄수화물", "단백질", "지방", "당류", "나트륨", "콜레스테롤", "포화지방산", "트랜스지방산" };
	...
	public String toString() {
		return "FoodNutriment [code=" + code + ", 제공량=" + serving_wt + ", 열량=" + nutr_cont1 + ", 탄수화물=" + nutr_cont2
				+ ", 단백질=" + nutr_cont3 + ", 지방=" + nutr_cont4 + ", 당류=" + nutr_cont5 + ", 나트륨=" + nutr_cont6
				+ ", 콜레스테롤=" + nutr_cont7 + ", 포화지방산=" + nutr_cont8 + ", 트랜스지방산=" + nutr_cont9 + "]";
	}
	...
	private static double getnutr(String input, int index) {
		...
		while (cur < get.length()) {
			...
			} else if ((get.charAt(cur) == '.' && !haveseendot)
					|| (get.charAt(cur) == ',' && Character.isDigit(get.charAt(cur + 1)))) {
				sb.append('.');
				if (!haveseendot)
					haveseendot = true;
			} else {
