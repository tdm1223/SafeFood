...
import com.edu.ssafy.dto.Blog;

public class BlogSearch {

	public List<Blog> search(String query) throws MalformedURLException, IOException {
		Document doc = Jsoup.connect("http://search.zum.com/search.zum?method=egloos.post&query="+query+"&qm=f_typing.egloos").get();		
		...
		for(Element blog : blogs) {
			...
			try {
				src = "http://"+blog.getElementsByTag("img").first().getElementsByAttribute("src").attr("src").substring(2);
